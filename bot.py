import telebot
import yt_dlp
import os
import re
import requests
import json
import time

TOKEN = "8912629367:AAGDvBUBFE97vLTPzlXsbmALJHi0u3XUCjs"  # ВСТАВЬ СВОЙ ТОКЕН

bot = telebot.TeleBot(TOKEN)

@bot.message_handler(commands=['start'])
def start(message):
    bot.reply_to(message, 
        "🎬 Привет! Я бот-скачивалка!\n\n"
        "Отправь мне ссылку на YouTube, и я скачаю видео!\n"
        "Если видео не скачается — я дам ссылку для скачивания."
    )

@bot.message_handler(func=lambda m: True)
def download_video(message):
    url = message.text.strip()
    
    if not url.startswith("http"):
        bot.reply_to(message, "❌ Это не ссылка! Отправь ссылку на YouTube.")
        return
    
    msg = bot.reply_to(message, "⏳ Начинаю скачивание...")
    
    # Извлекаем ID видео из ссылки
    video_id = None
    id_match = re.search(r'(?:v=|/)([0-9A-Za-z_-]{11})', url)
    if id_match:
        video_id = id_match.group(1)
    
    # ПРОБУЕМ СКАЧАТЬ ЧЕРЕЗ YT-DLP
    try:
        ydl_opts = {
            'format': 'best[height<=720]',
            'outtmpl': 'video.%(ext)s',
            'quiet': True,
            'no_warnings': True,
            'ignoreerrors': True,
            'no_check_certificate': True,
            'user_agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
        }
        
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = None
            
            # Пробуем скачать
            try:
                info = ydl.extract_info(url, download=True)
            except:
                pass
            
            # Если не получилось — пробуем через ID
            if info is None and video_id:
                try:
                    info = ydl.extract_info(f"https://www.youtube.com/watch?v={video_id}", download=True)
                except:
                    pass
            
            # Если всё равно не получилось — ошибка
            if info is None:
                raise Exception("Не удалось скачать")
            
            filename = f"video.{info['ext']}"
        
        # Отправляем видео
        with open(filename, 'rb') as f:
            bot.send_video(
                message.chat.id, 
                f, 
                caption=f"✅ Готово!\n📹 {info.get('title', 'Без названия')[:100]}"
            )
        os.remove(filename)
        return  # Всё хорошо, выходим
        
    except Exception as e:
        error_text = str(e)
        
        # Если ошибка "Sign in" или "NoneType" — пробуем запасной вариант
        if "Sign in to confirm" in error_text or "NoneType" in error_text or "not found" in error_text.lower():
            bot.edit_message_text(
                "⏳ YouTube не даёт скачать, пробую найти ссылку...",
                message.chat.id, msg.message_id
            )
            time.sleep(1)
            
            # ЗАПАСНОЙ ВАРИАНТ: через SaveFrom
            try:
                # Чистим ссылку
                clean_url = re.sub(r'[&?].*$', '', url)
                
                # Запрос к SaveFrom
                api_url = "https://api.savefrom.net/2/"
                params = {
                    'url': clean_url,
                    'ajax': 1,
                    'lang': 'ru'
                }
                
                response = requests.get(api_url, params=params, timeout=20)
                data = response.json()
                
                if 'result' in data and 'files' in data['result']:
                    files = data['result']['files']
                    
                    # Ищем ссылку на лучшее качество
                    download_link = None
                    best_quality = 0
                    
                    for quality, info in files.items():
                        if 'link' in info:
                            if 'quality_label' in info and info['quality_label']:
                                try:
                                    q = int(re.search(r'(\d+)p', info['quality_label']).group(1))
                                    if q > best_quality:
                                        best_quality = q
                                        download_link = info['link']
                                except:
                                    download_link = info['link']
                            else:
                                download_link = info['link']
                    
                    if download_link:
                        bot.edit_message_text(
                            f"✅ Нашёл ссылку для скачивания!\n\n"
                            f"🔗 Ссылка:\n{download_link}\n\n"
                            "📌 Открой ссылку в браузере, чтобы сохранить видео!",
                            message.chat.id, msg.message_id
                        )
                        return
                
                # Если не нашли ссылку
                bot.edit_message_text(
                    "❌ Не удалось найти ссылку для скачивания.\n\n"
                    "Попробуй:\n"
                    "1️⃣ Другую ссылку\n"
                    "2️⃣ Подождать 10-15 минут\n"
                    "3️⃣ Видео без возрастных ограничений",
                    message.chat.id, msg.message_id
                )
                
            except Exception as e2:
                bot.edit_message_text(
                    f"❌ Не удалось скачать видео.\n\n"
                    "Попробуй:\n"
                    "1️⃣ Другую ссылку\n"
                    "2️⃣ Подождать 10-15 минут\n"
                    "3️⃣ Отправить ссылку на другое видео\n\n"
                    f"Ошибка: {str(e2)[:100]}",
                    message.chat.id, msg.message_id
                )
        else:
            # Другие ошибки
            bot.edit_message_text(
                f"❌ Ошибка: {error_text[:150]}\n\n"
                "Попробуй другую ссылку или подожди.",
                message.chat.id, msg.message_id
            )

bot.polling()
